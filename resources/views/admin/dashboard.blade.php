<x-layouts.admin title="لوحة التحكم">
    <h1 class="text-xl sm:text-2xl font-bold mb-5 sm:mb-6">لوحة التحكم</h1>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
        ...
        <div class="bg-surface-raised border border-border rounded-2xl p-4 sm:p-5">
            ...
            <p class="text-xs sm:text-sm text-text-muted leading-snug">{{ $card['label'] }}</p>
        </div>
        @endforeach
    </div>
                ...
                <tr class="border-b border-border last:border-0">
                    <td class="px-4 sm:px-5 py-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="font-medium break-words">{{ $product->title_ar ?: $product->title }}</p>
                                <p class="text-xs text-text-muted break-words">{{ $product->category->name_ar ?: $product->category->name }}</p>
                            </div>
                        </div>
                    </td>
                    <td class="px-4 sm:px-5 py-3 text-end sm:text-start font-semibold text-brand whitespace-nowrap">{{ number_format($product->clicks_count) }} نقرة</td>
                </tr>
                @empty
        ...
        <table class="w-full text-sm">
            <tbody>
                ...
                <tr class="border-b border-border last:border-0">
                    ...
                    <td class="px-4 sm:px-5 py-3 text-end sm:text-start font-medium whitespace-nowrap">Rs. {{ number_format($product->price, 0) }}</td>
                </tr>
                @empty
